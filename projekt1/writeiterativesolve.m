function [] = writeiterativesolve(power_range, data_func, solver_func)
    max_iters = 100000;
    tolerance = 1e-9;
    ...
    for i = 1:length(power_range)
        ...
        [A, b] = data_func(n);
        tic;
        [x, iters] = solver_func(A, b, tolerance, max_iters);
        t = toc
        r = residuum(A, b, x);
        rnorm = norm(r)
        ...
        writecell(prepare_nums([n iters]), strcat('iters', outfile_suffix), 'Range', strcat('A', num2str(i), ':B', num2str(i)));
    end
end
